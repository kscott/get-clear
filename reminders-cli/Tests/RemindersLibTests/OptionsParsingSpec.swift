...
struct OptionsParsingTests {
    ...
    struct DateField {
        ...
        func takesBareDate() {
            let parsed = ParsedCommand(identifiers: [], bareDateRange: "friday", values: [:], trailingText: nil)
        ...
        func fallsBackToDueKeyword() {
            let parsed = ParsedCommand(identifiers: [], bareDateRange: nil, values: ["due": "friday"], trailingText: nil)
            #expect(parseOptions(from: parsed).date == "friday")
        }

        @Test("is empty when neither a bare date nor due is present")
        func emptyWhenAbsent() {
            let parsed = ParsedCommand(identifiers: [], bareDateRange: nil, values: [:], trailingText: nil)
            #expect(parseOptions(from: parsed).date == "")
        }

        @Test("passes 'due none' through as the literal string 'none'")
        func passesDueNoneThrough() {
            let parsed = ParsedCommand(identifiers: [], bareDateRange: nil, values: ["due": "none"], trailingText: nil)
        ...
        func passesDueOnFridayThrough() {
            let parsed = ParsedCommand(identifiers: [], bareDateRange: nil, values: ["due": "on friday"], trailingText: nil)
    ...
    struct EachKeyword {
        ...
        func recurrenceFromRepeat() {
            let parsed = ParsedCommand(identifiers: [], bareDateRange: nil, values: ["repeat": "monthly"], trailingText: nil)
            #expect(parseOptions(from: parsed).recurrence == "monthly")
        }

        @Test("priority comes from the priority keyword")
        func priorityFromKeyword() {
            let parsed = ParsedCommand(identifiers: [], bareDateRange: nil, values: ["priority": "high"], trailingText: nil)
        ...
        func urlFromKeyword() {
            let parsed = ParsedCommand(
                identifiers: [], bareDateRange: nil, values: ["url": "https://example.com"], trailingText: nil
            )
        ...
        func listFromKeyword() {
            let parsed = ParsedCommand(identifiers: [], bareDateRange: nil, values: ["list": "Bills"], trailingText: nil)
    ...
    struct NoteField {
        ...
        func fromTrailingText() {
            let parsed = ParsedCommand(identifiers: [], bareDateRange: nil, values: [:], trailingText: "buy milk")
