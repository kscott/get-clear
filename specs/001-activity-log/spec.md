...
**`what` and `recap` are Claude-ready.** Both commands are designed to serve two consumers equally: a person reading output directly at the CLI, and Claude calling the command via Bash to compose a narrative or answer a question. `recap` output is structured exactly for this — grouped by commitment type, tightly worded, no filler. The `get-clear` skill (get-clear #30) exposes these commands to Claude. "How's my day going?" is Claude calling `get-clear recap` and narrating the result.
