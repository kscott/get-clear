...
3. **Naming rule** — an identifier is exactly one token, quoted if spaced — including a search query (`reminders find "pick up milk"`). No greedy-to-keyword. Stray token → `unexpectedTokens` + "quote it?" hint. The trailing text field (`note`/`body`/`message`) is the only value where quotes are *optional* — but **docs and usage text quote it anyway** (FR-016), so the taught pattern is uniformly "quote every value."
