...
- [ ] T001 Confirm `./scripts/test` is green on the branch tip and note the pass count. No `Package.swift` change is needed — the parser is additive to `GetClearKit`, and `RemindersLib` already depends on `GetClearKit`. (Merge `main` first so this branch has spec 016's toolchain — Swift Testing, tools-version 6.0, `scripts/`.)
...
- [ ] T003 Create `Tests/GetClearKitTests/CommandArgumentsSpec.swift` — `@Suite("parseCommand")`, one assertion per `@Test`: required/optional/missing identifiers; a required identifier blocked by a keyword word → `missingIdentifier` naming the token; an optional identifier skips a leading keyword word (`list by created`); `.none` rejects a stray token; `.bareDate` captured verbatim; keyword values single- and multi-token; keyword order-independence (two permutations → equal `ParsedCommand`); trailing text captured to end including later keywords; a fully-quoted token stream ≡ its minimally-quoted equivalent; one `@Test` per `ArgumentError` case (`missingIdentifier`, `unexpectedTokens`, `unknownKeyword`, `missingValue`, `duplicateKeyword`, `dateGivenTwice`).
- [ ] T003a [P] Add a **new leading-`on` strip at the top of `parseDate`** in `Sources/GetClearKit/DateParser.swift` — after the `input.lowercased().trimmingCharacters` on line ~40, drop a leading `on ` token (~2 lines). Note: `at` is *not* a leading strip today (it lives inside the time-pattern regex), so this is a new addition, not a copy of an existing pattern. Verify `parseDate("on march 1") == parseDate("march 1")` and `parseDate("on friday") == parseDate("friday")`. Update `Tests/GetClearKitTests/DateParserSpec.swift` with `@Test`s for `on` before a month+day, a weekday, and a combined date+time (`on friday at 5pm`). This is the whole of FR-013's `on` handling.
...
- [ ] T030 [P] [US5] Add a decision-log entry to `ARCHITECTURE.md` — shared command parser in GetClearKit (`CommandArguments.swift`), the `CommandShape` descriptor, Phase 1 = reminders / Phase 2 = #192–195, and the deferred `lists`/`open` guard (#201).
