...
- [ ] T003a [P] Add a leading-`on` strip to `Sources/GetClearKit/DateParser.swift` — mirror the existing leading-`at` handling so `parseDate("on march 1") == parseDate("march 1")` and `parseDate("on friday") == parseDate("friday")`. Update `Tests/GetClearKitTests/DateParserSpec.swift` with `@Test`s for `on` before a month+day, a weekday, and a combined date+time (`on friday at 5pm`). This is the whole of FR-013's `on` handling.
