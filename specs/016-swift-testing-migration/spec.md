...
- **The `swift-tools-version` bump to 6.0 changes a manifest default.** With tools-version ≥ 6.0 the default language mode becomes v6; every target must explicitly pin Swift 5 mode so the code under test compiles with unchanged semantics.
- **A partially-migrated state on the branch.** While any target still imports Quick, the whole-suite `swift test` cannot build locally. Migrating and de-Quick-ing one target at a time keeps `swift build` green throughout and lets `swift test` run for already-migrated targets.
