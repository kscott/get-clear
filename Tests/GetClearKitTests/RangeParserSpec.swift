    ...
    override class func spec() {
        ...
        describe("parseRange") {
            ...
            context("weekday names") {
                ...
                it("each weekday resolves to a current or future date") {
                    for day in days {
            ...
            context("specific dates — short numeric format") {
            ...
            context("explicit ranges") {
                ...
                it("'yesterday to tomorrow' is not a single-day range") {
        ...
        describe("parseSingleDate") {
            ...
            it("'2026-03-15' resolves to a date") {
                expect(parseSingleDate("2026-03-15", cal: cal, now: Date())).toNot(beNil())
            }
            it("'3/15' resolves to a date") {
                expect(parseSingleDate("3/15", cal: cal, now: Date())).toNot(beNil())
            }
            it("returns nil for unrecognized input") {
        ...
        describe("formatRangeDescription") {
            it("single-day description is non-empty") {
                let desc = formatRangeDescription(parseRange("today")!)
                expect(desc).toNot(beEmpty())
            }
            it("single-day description includes a weekday name") {
                let desc = formatRangeDescription(parseRange("today")!)
                let names = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
                expect(names.contains(where: { desc.contains($0) })) == true
            }
            it("multi-day description is non-empty") {
                let desc = formatRangeDescription(parseRange("week")!)
                expect(desc).toNot(beEmpty())
            }
            it("multi-day description includes an en-dash") {
                let desc = formatRangeDescription(parseRange("week")!)
                expect(desc).to(contain("–"))
            }
        }
    }
}
