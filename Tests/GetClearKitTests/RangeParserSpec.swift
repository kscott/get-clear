...
struct RangeParserTests {
    ...
    struct SingleDayShorthands {
        ...
        @Test("'today' end is also today")
        func todayEndIsToday() {
            #expect(parseRange("today").map { sameDay($0.end, Date()) } == true)
        }
    ...
    struct WeekdayNames {
        ...
        @Test("each weekday resolves to a current or future date")
        func eachWeekdayCurrentOrFuture() {
            for day in days {
    ...
    @Suite("specific dates — ISO format")
    struct IsoFormat {
    ...
    @Suite("specific dates — short numeric format")
    ...
    struct ExplicitRanges {
        ...
        @Test("'yesterday to tomorrow' is not a single-day range")
        func yesterdayToTomorrowNotSingleDay() {
            #expect(parseRange("yesterday to tomorrow")?.isSingleDay == false)
        }
    }
...
struct ParseSingleDateTests {
    ...
    @Test("'yesterday' resolves to yesterday")
    func yesterdayResolves() throws {
        let expected = try #require(cal.date(byAdding: .day, value: -1, to: Date()))
        #expect(parseSingleDate("yesterday", cal: cal, now: Date()).map { sameDay($0, expected) } == true)
    }
    ...
    @Test("'2026-03-15' resolves to a date")
    func isoResolves() {
        #expect(parseSingleDate("2026-03-15", cal: cal, now: Date()) != nil)
    }

    @Test("'3/15' resolves to a date")
    func shortResolves() {
        #expect(parseSingleDate("3/15", cal: cal, now: Date()) != nil)
    }
...
struct FormatRangeDescriptionTests {
    @Test("single-day description is non-empty")
    func singleDayNonEmpty() throws {
        let desc = try formatRangeDescription(#require(parseRange("today")))
        #expect(!desc.isEmpty)
    }

    @Test("single-day description includes a weekday name")
    func singleDayIncludesWeekday() throws {
        let desc = try formatRangeDescription(#require(parseRange("today")))
        let names = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
        #expect(names.contains(where: { desc.contains($0) }))
    }

    @Test("multi-day description is non-empty")
    func multiDayNonEmpty() throws {
        let desc = try formatRangeDescription(#require(parseRange("week")))
        #expect(!desc.isEmpty)
    }

    @Test("multi-day description includes an en-dash")
    func multiDayIncludesEnDash() throws {
        let desc = try formatRangeDescription(#require(parseRange("week")))
