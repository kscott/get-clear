...
### 2026-09-04 — get-clear adopts the argument-shape rule (#202)

**Decision:** The sixth binary — `get-clear` itself (`Sources/GetClear/`), never covered by any Phase 2 issue — now goes through `parseCommand` via a new `GetClearCommandShapes.swift`. `recap` gets the same bare-range shape as calendar's `list`/`today`/`week`; `setup`/`update`/`check-update` get the same zero-argument stray-token guard the other five tools' no-arg commands already have, via a shared `validateNoArguments(_:shape:)` helper. `what` is deliberately untouched — consolidating it is #197's job, not this issue's.

**New `GetClearTests` target** — `get-clear` never had one before. `@testable import GetClear` works fine for an executable target under SwiftPM 6.0. Only the one new file this issue added (`GetClearCommandShapes.swift`) has direct tests; the rest of `Sources/GetClear/` shows 0% coverage, same as it did before this issue (just newly visible) — `get-clear` never went through the business-logic-extraction project the five tools got (#35-39), so its handlers mix EventKit/network/interactive-I/O directly with no injectable seam. Filed as **#203**, explicitly out of scope here — a project on the scale of #35-39, not a quick fix.

**Handlers converted from `async` (calling `fail()`/`exit()` inline) to `async throws`**, matching the confirmed suite-wide convention: handlers throw, only the CLI entry point (`main.swift`) calls `fail()`. Verified by grep that `fail(` appears only in each tool's `main.swift` — every actual handler throws its domain error instead. Introduced `GetClearError` (a plain struct + `LocalizedError`, matching contacts/text/mail's shape rather than reminders'/calendar's older `CustomStringConvertible`/no-conformance shapes) as the wrapError target and the type every remaining `fail()` call in `RecapCommand.swift`/`UpdateCommand.swift` now throws instead.

**Drive-by lint fixes** (Ken's explicit request, same discipline as #193/#195/#194): 2 `force_unwrapping` (a `Calendar.date(byAdding:)` converted to a graceful fallback since it fails only in practically-impossible overflow cases; a malformed-URL-from-GitHub case converted to a loud `throw` since that's a real, user-visible failure mode) and 1 `cyclomatic_complexity` in `RecapFormatter.swift`'s `formatRecap`, fixed by extracting `formatMultiDayRecap`/`dayGroups` — two genuinely separate responsibilities (which days exist vs. what belongs to a given day) that were tangled in one function. Also fixed a real (non-SwiftLint) compiler warning: `main.swift`'s `default: usage()` discarded `usage()`'s return value, meaning an unrecognized-for-get-clear command printed nothing at all — now `print(usage())`.

**Discovered along the way, filed as follow-ups, not fixed here:** error types have drifted into three incompatible shapes across the six binaries (plain struct, `CustomStringConvertible`, `LocalizedError`) — **#204** proposes unifying on `LocalizedError` (the Foundation-standard shape, already used by 4 of 6 tools, and the one that needs the least code since `main.swift`'s explicit typed catches become unnecessary once a type conforms).

**Coverage:** `Sources/GetClear/` — one new fully-tested file (`GetClearCommandShapes.swift`, `GetClearError.swift`), rest at the same 0% it always was (see #203). Suite: 1282/1282 tests passing, lint 29 warnings (down from 32 — zero left in `Sources/GetClear/`).

### 2026-09-04 — Mail adopts the argument-shape rule; `--draft` retired (#194, spec 015 Phase 2)

**Decision:** Mail's four dispatched commands (`send`, `draft`, `find`, `open`) go through `parseCommand` via `MailCommandShapes.swift`. `draft` is a new top-level command — `mail draft "<to>" ... body "<text>"` — sharing `send`'s exact shape (`MailCommandShapes.draft = .send`) and composition logic (`SendHandler.swift`'s private `compose(...)`); only the final action (`client.send` vs `client.saveDraft`) and confirmation wording differ. `--draft` is deleted. This closes design.md's one open flag violation ("`--draft` in mail is the current open violation... The fix is `mail draft`") — not a new decision, just finally implementing one already on record, done now because rewriting this exact parser was the natural moment.

**GetClearKit gains `Keyword(repeatable:)` and `ParsedCommand.repeatedValues`:** mail's `cc` and `attach` can each be given more than once in one command (`cc bob cc carol`, `attach a.pdf attach b.pdf`) — both real, tested, intentional behavior predating this issue. Confirmed with Ken this is a genuinely different situation from contacts' `change` ([[feedback-change-parsing-stays-separate]]): `cc`/`attach` repeats are the *same kind of value* at higher cardinality (a string, one per occurrence), not a different value shape needing operation semantics (add/remove/replace/clear). That distinction is why this got a shared parser extension where contacts' case did not — extending the model one dimension (cardinality) is not the same move as bolting on a different model (operations). A `Keyword` opts in with `repeatable: true`; `parseCommand` then appends to `ParsedCommand.repeatedValues[canonical]` instead of throwing `duplicateKeyword` on the second occurrence. Additive — every existing shape and every existing `ParsedCommand` construction is unaffected (`repeatedValues` defaults to `[:]`).

**Also discovered mid-issue:** `to`'s and `cc`'s "send to multiple people" already works today, but only via a Contacts **group** name expanding to every member (`resolveRecipients`'s group-lookup branch) — not via ad-hoc multiple names/emails in one command. That ad-hoc case is #68 (deferred, unrelated to this issue) — Ken confirmed he works around it with groups today, which is exactly the mechanism this issue leaves untouched.

**`body`'s required-and-non-empty check moved from a hand-rolled guard into the shared parser**, via `CommandShape.requiresTrailingText` (added in #195 anticipating exactly this — mail's `body` was the second occurrence, text's `message` the first). `SendHandler.swift` no longer has its own `guard !bodyText.isEmpty` — `parseCommand` throws `ArgumentError.missingValue(keyword: "body")` itself before the handler's business logic ever runs.

**Two lint issues introduced then fixed within this same change** (caught by `./scripts/lint` before commit, per the discipline the #193/#195 sessions established): the private `compose(...)` helper hit `function_parameter_count` (6 params) — fixed by dropping the redundant `shape` parameter, since `.draft` and `.send` are definitionally the same value; and `loadGroupMembers`'s `for` loop with a single `if` inside — pre-existing, copied verbatim during the rewrite rather than actually fixed as planned — converted to `for ... where`.

**Coverage:** MailLib was already at 99.5%/98.5%/98.1% (lines/regions/functions) before this issue — the highest baseline of any tool in Phase 2. Landed at 99.5%/97.8%/96.7%; the small dip is new surface (`composeMessage`, `MailError.badArguments`), both covered except one confirmed compiler-artifact closure (same category as #193's ContactsLib findings — verified real vs. artifact by checking the underlying line actually executes).
...
**Why:** `find`'s query/range split was genuinely ambiguous — it guessed whether trailing tokens were part of the query or a range, trying `parseRange` on different token spans until one worked, silently absorbing an unparseable range into the query as a last resort. `show`/`remove`'s optional range had the same silent-fallback hole in reverse: an unparseable range string fell back to the default 30-day window instead of erroring. `next`'s count (`Int(args[1]) ?? 5`) silently defaulted to 5 on any non-numeric input. All three are now `CalendarHandlerError`s naming the bad value (FR-024), and `find`'s query is a plain one-token identifier — quoted if multi-word — same rule as everywhere else, no more guessing.
