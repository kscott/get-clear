...
func makeEvent(
    ...
    base.year = 2026; base.month = 4; base.day = 29 + dayOffset
    ...
    if isAllDay {
        ...
        var sc = base; sc.hour = startHour; sc.minute = 0
        var ec = base; ec.hour = endHour;   ec.minute = 0
...
final class SpyCalendarStore: CalendarStore {
    ...
    func fetchCalendars() async throws -> [CalendarItem]                               { calendars }
    func fetchEvents(in range: DateInterval, calendarIdentifiers: [String]?) async throws -> [EventItem] { events }
    func add(_ item: EventItem) async throws -> EventItem                              { addedItems.append(item); return item }
    func remove(identifier: String) async throws                                       { removedIds.append(identifier) }
}
