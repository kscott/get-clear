...
//   Explicit range: "<date> to <date>"   (any two single-date expressions above)
...
public func parseRange(_ input: String) -> ParsedRange? {
    ...
    if s.range(of: #"^\d+d$"#, options: .regularExpression) != nil {
        ...
        return ParsedRange(start: start, end: endOfDay(end, cal: cal), isSingleDay: n == 1)
    }
    ...
    switch stripped {
    case "week":
        let start = weekStart(of: now, cal: cal)
        let end   = cal.date(byAdding: .day, value: 6, to: start)!
        return ParsedRange(start: start, end: endOfDay(end, cal: cal), isSingleDay: false)
    case "next week":
        let start = cal.date(byAdding: .weekOfYear, value: 1, to: weekStart(of: now, cal: cal))!
        let end   = cal.date(byAdding: .day, value: 6, to: start)!
        return ParsedRange(start: start, end: endOfDay(end, cal: cal), isSingleDay: false)
    case "last week":
        let start = cal.date(byAdding: .weekOfYear, value: -1, to: weekStart(of: now, cal: cal))!
        ...
        return ParsedRange(start: start, end: endOfDay(end, cal: cal), isSingleDay: false)
    case "month":
        let (start, end) = monthBounds(of: now, cal: cal)
    ...
    case "next month":
        ...
        let (_, end) = monthBounds(of: firstOfNext, cal: cal)
    ...
    case "last month":
        let firstOfLast = cal.date(from: cal.dateComponents([.year, .month],
                         from: cal.date(byAdding: .month, value: -1, to: now)!))!
        let (_, end) = monthBounds(of: firstOfLast, cal: cal)
...
/// Parse a single date reference — used both for standalone ranges and each side of "X to Y".
...
private func monthBounds(of date: Date, cal: Calendar) -> (Date, Date) {
    let start  = cal.date(from: cal.dateComponents([.year, .month], from: date))!
    let end    = cal.date(byAdding: DateComponents(month: 1, second: -1), to: start)!
    return (start, endOfDay(end, cal: cal))
}
